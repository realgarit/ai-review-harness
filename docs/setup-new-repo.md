...
1. Copy into the new repo:
   - `.github/workflows/ai-review.yml` (GitHub; Semgrep gate plus native
     Codex review configured in Codex Cloud) or
     `.gitea/workflows/ai-review.yml` (Gitea; configurable dispatcher)
   - `scripts/compute-diff.sh`
   - `scripts/run-semgrep.sh`
   - For Gitea or local dispatcher use: `scripts/run-ai-review.sh`,
     `scripts/invoke-model.sh`, `scripts/format-review-comment.py`, and
     `prompts/review-prompt.md`
2. For Gitea or local dispatcher use, choose your AI model. The harness
   supports multiple providers — add the corresponding secrets to the repo
   (Settings > Secrets and variables > Actions, or Gitea's equivalent):

   | `AI_MODEL`     | Secrets needed                                        | Notes                                    |
   | -------------- | ----------------------------------------------------- | ---------------------------------------- |
   | `codex`        | None for GitHub native review; provider credentials for Gitea/local use | Default. Local ChatGPT login; GitHub uses native Codex Code Review. |
   ...
   Set `AI_MODEL` in `.ai-review.conf` or as a Gitea workflow variable to
   switch the provider used by the direct dispatcher. GitHub's checked-in
   workflow does not select a provider: native Codex Code Review is configured
   separately in Codex Cloud. The optional `OPENAI_MODEL`, `DEEPSEEK_MODEL`,
   and similar variables apply to local or Gitea dispatcher runs.
   ...
   ChatGPT**. `codex exec` reuses that saved login. On GitHub, enable native
   Codex Code Review in Codex Cloud for the repository. The checked-in GitHub
   workflow runs only the deterministic Semgrep gate and does not pass an API
   key to a checkout shell or provider action. The Gitea template has no
   bundled equivalent proxy and is therefore for trusted runners only; it
   keeps the configurable direct dispatcher and its provider-specific secrets.
   Never commit or copy `~/.codex/auth.json` into a public or untrusted runner.
...
3. On GitHub, the workflow's `docker run semgrep/semgrep` step needs
   ...
   on this step.
4. Open a test PR. On GitHub, confirm both the Semgrep comment and the native
   Codex review status. On Gitea, confirm the combined Semgrep and AI comment.
